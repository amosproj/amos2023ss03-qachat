...
class DocumentEmbedder:
    ...
    def store_information_in_database(self, typ: DataSource):
        ...
        last_updated_database_result = self.supabase.table("last_update_per_data_typ").select("last_update").eq("type",
                                                                                                                typ.value).execute()
        if len(last_updated_database_result.data) == 0:
            last_updated = datetime(1970, 1, 1)
        else:
            last_updated = parser.parse(last_updated_database_result.data[0]["last_update"]).replace(tzinfo=None)

        current_time = datetime.now()
        all_changed_data = data_preprocessor.load_preprocessed_data(current_time, last_updated)

        if len(all_changed_data) != 0:
            ids = {data.id for data in all_changed_data}
            self.supabase.rpc("delete_old_embeddings", {"ids": list(ids), "delete_type": typ.value}).execute()

            self.vector_store.add_texts(
                [data.text for data in all_changed_data],
                [{"last_changed": data.last_changed.isoformat(), "type": typ.value, "id": data.id} for data in
                 all_changed_data], )
