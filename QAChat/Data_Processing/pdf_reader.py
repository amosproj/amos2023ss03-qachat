...
class PDFReader:
    def read_pdf(self, pdf_object):
        # First try to read the PDF with PDFMiner
        text = self.__read_pdf_from_object(pdf_object)

        # If the result is empty or too short, try OCR
        if len(text.strip()) < 100:
            text = self.__ocr_pdf(pdf_object)

        return text

    def __read_pdf_from_object(self, pdf_object):
...
if __name__ == "__main__":
    ...
    with open("../../Deliverables/sprint-03/planning-documents.pdf", "rb") as f:
        pdf_bytes = f.read()
    print(reader.read_pdf(pdf_bytes))
