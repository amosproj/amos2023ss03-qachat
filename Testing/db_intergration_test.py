...
class DatabaseIntegrationTestDBExist(unittest.TestCase):
    def setUp(self):
        ...
        self.supabase_client = supabase.create_client(os.environ.get("SUPABASE_URL"), 
    ...
    def test_if_data_embedding_exists(self):       
        ...
        self.assertEqual(str(type(response)).split("'")[1], 
    ...
    def test_if_last_update_per_data_typ_exists(self):
        ...
        response = self.supabase_client.table("last_update_per_data_typ").select("*").execute()
        # Asserting that the response type is "postgrest.base_request_builder.APIResponse"
        self.assertEqual(str(type(response)).split("'")[1], 
                         "postgrest.base_request_builder.APIResponse")
...
class DatabaseIntegrationTestDBInteract(unittest.TestCase):
    ...
    def test_db_interaction(self):
        test_obj = {
            ...
            "metadata": {"last_changed": "01.01.1000", "type": "dummy", "id":"999999999999999999"}, 
            "embedding": [0]*768
            }
        ...
        result =  self.supabase_client.table("data_embedding").select('content').eq('content', 'JUST A TEST OBJECT').execute()
        ...
        result = self.supabase_client.table("data_embedding").delete().eq("content", 'JUST A TEST OBJECT').execute()
