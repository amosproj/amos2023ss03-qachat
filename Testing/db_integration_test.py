...
class DatabaseIntegrationTestDBExist(unittest.TestCase):
    ...
    def test_if_last_update_per_data_typ_exists(self):
        ...
        response = self.weaviate_client.query.get("LastChanged")
...
class DatabaseIntegrationTestDBInteract(unittest.TestCase):
    ...
    def test_db_interaction(self):
        ...
        result = (
            self.weaviate_client.query.get("Embeddings", properties=["type_id"])
            .with_where(
                {
                    "path": "text",
                    "operator": "Equal",
        ...
        self.assertEqual(
            result["data"]["Get"]["Embeddings"][0]["type_id"], "999999999999999999"
        )
        # Selecting the object from the "data_embedding" table based on the content
        result = (
            self.weaviate_client.query.get("Embeddings", properties=["text"])
            .with_where(
                {
                    "path": "text",
                    "operator": "Equal",
        ...
        self.assertEqual(
            result["data"]["Get"]["Embeddings"][0]["text"], "JUST A TEST OBJECT"
        )
